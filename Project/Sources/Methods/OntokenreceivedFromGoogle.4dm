...
var $events:=GetEvents_Google($windowRef; $OAuth2)

var $caldendarEvents:=[]

// Create event object for calendar interface usage
For each ($event; $events)
	var $ev:={id: $event.id; title: $event.summary; start: $event.start.dateTime; end: $event.end.dateTime; allDay: $event.isAllDay}
	$caldendarEvents.push($ev)
End for each 

CALL FORM:C1391($windowRef; Formula:C1597(Form:C1466.GoogleAccessorManager.events:=$1); $caldendarEvents)
CALL FORM:C1391($windowRef; Formula:C1597(WA OPEN URL:C1020(*; "GoogleWebArea"; $page)))
